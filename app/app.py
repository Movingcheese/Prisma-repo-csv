...
def get_github_enterprise_repos(api_url, jwt_token):
    headers = {
...
def generate_csv(repos, output_file):
    if not repos:
        print("Response: No GitHub Enterprise repositories to write to CSV.")
        return
...
if jwt_token:
    print("Authentication successful. Proceeding to fetch GitHub Enterprise repositories.")
    github_enterprise_repos = get_github_enterprise_repos(API_URL, jwt_token)
    if github_enterprise_repos:
        print(f"Found {len(github_enterprise_repos)} GitHub Enterprise repositories.")
        generate_csv(github_enterprise_repos, OUTPUT_FILE)
    else:
        print("No GitHub Enterprise repositories found or failed to retrieve repositories.")
else:
